        ...
        **The objective:** Build a data pipeline + reporting system that automatically tracks loan performance, classifies DPD buckets, measures lender exposure, and produces audit-ready monthly reports.
        ...
        **DPD Logic:**
        ...
        These are the questions your analysis must answer. Do not look up solutions — work through each independently.
        ...
        6. Calculate the DPD for every transaction. Classify each into: Standard (0), SMA-0 (1–30), SMA-1 (31–60), SMA-2 (61–90), NPA (90+). How many EMIs fall in each bucket?
        ...
        24. Build a risk scorecard for each lender using: default rate, overdue ratio, and collection efficiency.
        ...
        26. Build a month-on-month cohort analysis: for loans disbursed in each month, what % went overdue within 90 days?
        27. Forecast next 3 months of expected collections using historical monthly trends (Python: linear regression or moving average).
        28. Flag any loan as "High Risk" if it meets 2+ of: Default status, DPD > 60 on any EMI, principal > ₹3,00,000.
        29. Create an investor MIS summary table: lender-wise disbursed, collected, overdue, default rate — formatted for a board deck.
        30. Build a mock audit trail: list all data anomalies found across Q16–Q20 in a single summary DataFrame.

        ---

        ## Looker Studio Dashboard Layout

        Design your Looker Studio report with **4 pages**:

        ---

        ### Page 1 — Executive Summary

        **Purpose:** One-screen snapshot for senior management.
        ...
        **Charts:**
        - Bar chart: Monthly Disbursal Trend (last 12 months)
        - Donut chart: Loan Status Distribution (Active / Closed / Default)
        - Bar chart: Disbursed Amount by Lender
        - Bar chart: Disbursed Amount by Loan Type

        **Filters:** Lender Name, Loan Type, Date Range (disbursal_date)

        ---

        ### Page 2 — DPD & Delinquency Analytics

        **Purpose:** Risk team view — who owes, how late, and how bad.

        **Scorecards:**
        - Total Overdue Loans
        - NPA Loan Count (DPD 90+)
        - Delinquency Ratio %
        - % EMIs Paid on Time

        **Charts:**
        - Stacked bar chart: DPD Bucket Distribution (count of EMIs per bucket)
        - Bar chart: DPD Bucket by Lender (side-by-side comparison)
        - Line chart: DPD Trend Over Time (monthly average DPD)
        - Table: Top 20 Loans by Highest DPD with loan_id, lender, status, max DPD

        **Filters:** Lender Name, Loan Type, Payment Status, DPD Bucket

        ---

        ### Page 3 — Lender Performance & Risk

        **Purpose:** Lender relationship management and risk exposure monitoring.

        **Scorecards (per lender — use filters):**
        - Lender Exposure %
        - Recovery Rate %
        - Overdue Ratio %
        - Avg Interest Rate
        ...
        **Charts:**
        ...
        - Stacked bar chart: Loan Status Breakdown by Lender (Active/Closed/Default)
        - Scatter plot: Lender Exposure vs. Default Rate
        - Table: Lender Risk Scorecard (disbursed, collected, default rate, overdue ratio — all lenders)

        **Filters:** Lender Name, Loan Type, Month

        ---

        ### Page 4 — Reconciliation & Data Quality

        **Purpose:** Finance ops and audit support.
        ...
        **Charts:**
        - Table: Loans with no transactions (loan_id, lender, status, principal)
        - Table: Flagged overpayment transactions
        - Bar chart: Monthly Missed Payment Count
        - Pie chart: Transaction Status Split (Paid / Delayed / Missed)

        **Filters:** Lender Name, Payment Status, Flag Type
        ...
        │
        ├── data/
        │   ├── loans.csv                    ← Master loan register (500 loans)
        │   ├── transactions.csv             ← EMI payment records (~8,000 rows)
        │   └── lender_summary.csv          ← Aggregated lender snapshot
        │
        ├── sql/
        │   ├── 01_schema.sql                ← Table creation scripts
        │   ├── 02_portfolio_kpis.sql        ← Section A queries
        │   ├── 03_dpd_analysis.sql          ← Section B queries
        │   ├── 04_collections.sql           ← Section C queries
        │   ├── 05_reconciliation.sql        ← Section D queries
        │   └── 06_lender_risk.sql           ← Section E queries
        │
        ├── python/
        │   ├── 01_data_cleaning.ipynb       ← Null handling, type fixes, validation
        │   ├── 02_dpd_buckets.ipynb         ← DPD classification logic
        │   ├── 03_portfolio_analysis.ipynb  ← KPI computation, segmentation
        │   ├── 04_reconciliation.ipynb      ← Data quality checks
        │   └── 05_advanced_analysis.ipynb   ← Cohort, forecasting, risk scoring
        │
        ├── dashboard/
        │   └── looker_studio_link.txt       ← Link to published Looker Studio report
        │
        ├── reports/
        │   ├── lender_risk_scorecard.xlsx   ← Exported lender risk table
        │   └── investor_mis_report.xlsx     ← Board-ready MIS summary
        │
        ├── screenshots/
        ...
        ├── README.md                        ← This file
        └── presentation/
        └── Analytics_Deck.pdf           ← Project walkthrough deck
        ```
        ...
        **Q: What was the most complex part?**  
        The DPD classification and reconciliation logic. DPD isn't just a subtraction — you have to handle missed payments (no payment date), partial payments, and edge cases where data quality breaks the logic.

        **Q: How did you validate data quality?**  
        I built a reconciliation notebook that flagged: loans with no transactions, payments exceeding EMI amounts, mismatched DPD values for Missed records, and customers with multiple active loans.

        **Q: What did the Looker Studio dashboard show?**  
        Four pages: Executive Summary (portfolio health), DPD Analytics (delinquency risk), Lender Performance (exposure and recovery), and Reconciliation (audit flags). Each page was designed for a different stakeholder.

        **Q: What would you add with more time?**  
        A default prediction model using logistic regression on DPD history, loan type, and tenure. Also automated monthly closing emails to lender relationship managers via Python.
